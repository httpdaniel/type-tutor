...
def get_predicted_text(predictions, temperature):
    most_common_predictions = predictions * np.array(prob_a)
...
generated_text = random.choices(list(characters), weights=np.array(first_letter_a) * np.array(prob_a), k=1)[0]
